...
9. 除了判断趋势强弱，你还必须明确回答“如果要参与，这笔机会在技术面上最合理的参与方式是什么”。
10. 你的分析窗口优先聚焦未来 1-3 个月，不要只停留在历史回顾。
...
## 分析基准信息
- 必须单独输出以下 3 项：
- `分析日期`：直接写本次分析使用的日期 `{normalized_trade_date}`
- `分析截面`：明确说明本报告判断基于哪个时间截面，例如“基于 {normalized_trade_date} 附近最新可得市场数据”或“基于 {normalized_trade_date} 前一个交易日收盘后的价格与指标”
- `截面价格`：必须给出本次分析最核心的参考价格，并说明该价格对应的是当前价格、最近收盘价还是分析基准价；如果数据中无法确认唯一价格，也要明确说明
