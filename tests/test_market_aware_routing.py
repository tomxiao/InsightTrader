...
class MarketAwareRoutingTests(unittest.TestCase):
    ...
    def test_route_to_vendor_uses_market_specific_vendor_for_hk_fundamentals(self):
        with patch.dict(
            ...
            {"akshare": lambda ticker, *_args, **_kwargs: ticker},
            clear=False,
        ):
    ...
    def test_global_news_uses_runtime_context_market(self):
        ...
        with patch.dict(
            VENDOR_METHODS["get_global_news"],
            {"finnhub": lambda *_args, **_kwargs: "finnhub-global"},
            clear=False,
        ):
    ...
    def test_route_emits_trace_for_market_specific_vendor(self, emit_trace_event):
        ...
        self.assertEqual(result, "600519.SH")
        decision_call = next(
            call for call in emit_trace_event.call_args_list if call.args[1] == "route.decision"
        )
        ...
        result_call = next(
            call for call in emit_trace_event.call_args_list if call.args[1] == "route.result"
        )
    ...
    @patch("tradingagents.dataflows.interface.emit_trace_event")
    def test_route_emits_category_fallback_trace_when_market_routing_disabled(
    ...
    def test_route_marks_returned_error_string_as_error_outcome(self, emit_trace_event):
        with patch.dict(
            VENDOR_METHODS["get_stock_data"],
            {
                "tushare": lambda *_args, **_kwargs: (
                    "Error retrieving stock data for 600519 via tushare: boom"
                )
            },
            clear=False,
        ):
