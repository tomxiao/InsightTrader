...
- 支持的建议行动：
  ...
  - `择机买入` -> 等待入场区间触发
  - `保持观望` -> 不交易
  - `建议卖出` -> 若已有多头仓位，则按下一个交易日开盘价卖出 1 股；空仓时跳过，不做空
...
3. 每个买入信号默认买入 `1` 股，并形成一个独立多头仓位。
4. `建议卖出`：若当前已有多头仓位，则按 FIFO 规则卖出最早开仓的 `1` 股，成交价使用卖出信号后的下一个交易日开盘价；若当前仓位为 `0`，则该卖出信号跳过。
5. 若入场前先出现明确失效价位被收盘跌破，则该买入信号失效。
6. 开仓后若价格触发失效价位，则按失效价止损。
7. 若未被 `sell` 或止损提前关闭，则持有到 `max_holding_days` 或回测窗口结束。
...
- `trades.csv`
- `summary.json`
- `MMdd-HHmm-ticker-bt.png`
- `MMdd-HHmm-ticker-ohlcv.csv`
...
- `MMdd-HHmm-ticker-bt.png`：自动生成的回测结果图
- `MMdd-HHmm-ticker-ohlcv.csv`：本次回测使用的本地 OHLCV 快照，结果图固定依赖这份文件绘制
...
- `择机买入` 按报告里的入场区间触发
- `保持观望` 直接跳过
- `建议卖出` 只会平掉已有多头仓位，不会开空仓
- `sell` 的执行价格固定为卖出信号后的下一个交易日开盘价
- 当 `--report` 来自同一个批次目录时，回测结果默认写回该批次目录根部
- 只有无法推断批次目录时，才会新建 `backtest/output/MMdd-HHmm-ticker/`
- 结果图绘制默认依赖当前目录下的本地 `*-ohlcv.csv`，避免因为重新拉行情导致图表漂移
